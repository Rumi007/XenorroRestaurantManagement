...
class HtmlViewerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: ResponsiveHelper.isDesktop(context) ? PreferredSize(child: WebAppBar(), preferredSize: Size.fromHeight(120))  : ResponsiveHelper.isMobilePhone() ? null : AppBarBase(
        title: getTranslated(htmlType == HtmlType.TERMS_AND_CONDITION ? 'terms_and_condition'
            : htmlType == HtmlType.ABOUT_US ? 'about_us' : htmlType == HtmlType.PRIVACY_POLICY ? 'privacy_policy' : 'terms_and_condition', context,
        ),
      ),
                  ...
                  child: Column(
                    children: [
                      ...
                      ResponsiveHelper.isDesktop(context)
                          ? Text(htmlType.toString().replaceAll('_', ' '),style: poppinsBold.copyWith(fontSize: 28,color: ColorResources.getTextColor(context)))
                          : SizedBox.shrink(),
                      Padding(
